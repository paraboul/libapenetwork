...
__inline static void ape_socket_release_data(
    unsigned char *data, ape_socket_data_autorelease data_type) {
    switch (data_type) {
...
ape_socket *APE_socket_new(uint8_t pt, int from, ape_global *ape) {
    ...
    if ((sock == 0 && (sock = socket((pt == APE_SOCKET_PT_UNIX
                                          ? AF_UNIX
...
#error "TCP KeepAlive not supported"
    ...
    if (setsockopt(socket->s.fd, SOL_SOCKET, SO_KEEPALIVE, &enable,
        ...
        APE_ERROR("libapenetwork",
                  "[Socket] Failed to set socket timeout (SO_KEEPALIVE) : "
                  "error %d %s\n",
    ...
    if (setsockopt(socket->s.fd, IPPROTO_TCP, KEEPALIVE_OPT, &secs,
                   sizeof(secs)) == -1) {
        APE_ERROR("libapenetwork",
                  "[Socket] Failed to set socket timeout (TCP_KEEPALIVE) : "
                  "error %d %s\n",
                  errno, strerror(errno));
        return 0;
    }
    ...
    if (setsockopt(socket->s.fd, IPPROTO_TCP, TCP_KEEPINTVL, &keepintvl,
                   sizeof(keepintvl)) == -1) {
        APE_ERROR("libapenetwork",
                  "[Socket] Failed to set socket timeout (TCP_KEEPINTVL) : "
                  "error %d %s\n",
                  errno, strerror(errno));
        return 0;
    }
    ...
    if (setsockopt(socket->s.fd, IPPROTO_TCP, TCP_KEEPINTVL, &kepcnt,
                   sizeof(kepcnt)) == -1) {
        APE_ERROR("libapenetwork",
                  "[Socket] Failed to set socket timeout (TCP_KEEPCNT) : error "
                  "%d %s\n",
                  errno, strerror(errno));
        return 0;
    }
    ...
    if (setsockopt(socket->s.fd, IPPROTO_TCP, TCP_USER_TIMEOUT, &mstimeout,
                   sizeof(mstimeout)) == -1) {
        APE_ERROR("libapenetwork",
                  "[Socket] Failed to set socket timeout (TCP_USER_TIMEOUT) : "
                  "error %d %s\n",
                  errno, strerror(errno));
        return 0;
    }
...
int APE_socket_listen(ape_socket *socket, uint16_t port, const char *local_ip,
    ...
    if (reuse_port && setsockopt(socket->s.fd, SOL_SOCKET, SO_REUSEPORT,
        ...
        APE_WARN("libapenetwork",
                 "[Socket] setsockopt (SO_REUSEPORT) (%s:%u) warning : %s\n",
...
#ifdef _WIN32
        APE_ERROR("libapenetwork",
                  "[Socket] Unix socket are not supported on Windows\n");
        return -1;
#else
...
retry_connect:
    if (socket->local_port != 0) {
        ...
        if (bind(socket->s.fd, (struct sockaddr *)&addr_loc,
                 sizeof(addr_loc)) == -1) {
        ...
        if (APE_SOCKET_IS_LZ4(socket, tx)) {
            ...
            */
            ...
            for (int cur_block = 0; cur_block < number_of_blocks; cur_block++) {
                ...
                if (cmp_len <= 0) {
                    APE_ERROR("libapenetwork",
                              "[Socket] LZ4 compression error %d\n", cmp_len);
                    return -1;
                }
            }
                ...
                rewrite:
                    if ((n = writev(socket->s.fd, chunks, i)) == -1) {
                        if (errno == EAGAIN) {
                            ...
                            APE_ERROR(
                                "libapenetwork",
                                "[Socket] writev() IO error: disconnect\n");
                            APE_socket_shutdown_now(socket);
                            return -1;
                        }
                    }
...
static int ape_socket_queue_data(ape_socket *socket, unsigned char *data,
    ...
    if (socket->max_buffer_memory_mb != 0 &&
        ...
        APE_ERROR("libapenetwork",
                  "[Socket] Maximum buffer size reached for socket %d\n",
                  socket->s.fd);
        APE_socket_shutdown_now(socket);
    }
    ...
    */
    ...
    while (pLen > 0) {
        ...
        if (socket->lz4.rx.decompress_position < sizeof(int)) {
            buffer_pos =
                ape_min(pLen, sizeof(int) - socket->lz4.rx.decompress_position);
        ...
        if (pLen > buffer_pos &&
            ...
            if (rc <= 0) {
                APE_ERROR("libapenetwork",
                          "[Socket] LZ4 Decompression error %d\n", rc);
                return -1;
            }
            ...
            APE_ERROR("libapenetwork",
                      "[Socket] io_error from block size (%d)\n",
                      socket->lz4.rx.current_block_size);
            return -1;
...
int ape_socket_read(ape_socket *socket) {
    ...
    do {
        ...
        socket_reread:
            nread =
                read(socket->s.fd, socket->data_in.data + socket->data_in.used,
                     socket->data_in.size - socket->data_in.used);
    ...
    if ((nread == 0 || io_error) &&
        ...
        if (io_error) {
            APE_ERROR("libapenetwork",
                      "[Socket] Socket %d disconnected because of IO error\n",
                      socket->s.fd);
        }
        ape_socket_destroy(socket);
...
static ape_socket_jobs_t *ape_socket_job_get_slot(ape_socket *socket,
                                                  int type) {
